import fs$8, { existsSync, promises, statSync as statSync$1, realpathSync, accessSync, constants as constants$5, readFileSync } from 'node:fs';
import fsp from 'node:fs/promises';
import os$1, { homedir } from 'node:os';
import { logger, useNuxt, addPlugin, addTemplate, addVitePlugin } from '@nuxt/kit';
import { colors } from 'consola/utils';
import { join, dirname as dirname$1, resolve, isAbsolute as isAbsolute$1 } from 'pathe';
...
import path$a, { dirname, join as join$1, relative, parse as parse$4 } from 'node:path';
import { hash } from 'ohash';
import { runtimeDir, clientDir, packageDir, isGlobalInstall } from '../dirs.mjs';
import { createBirpcGroup } from 'birpc';
import { stringify as stringify$5, parse as parse$5 } from 'structured-clone-es';
  ...
  async function readBuildInfo() {
    ...
    builds = await Promise.all(files.map(async (file) => {
      ...
      return {
        ...json,
        features: {
          bundleClient: fs$8.existsSync(join(dir, "client.html")),
          bundleNitro: fs$8.existsSync(join(dir, "nitro.html")),
          viteInspect: fs$8.existsSync(join(dir, ".vite-inspect"))
        }
      };
    }));
 ...
 */
function read$4(jsonPath, {base, specifier}) {
  ...
  try {
    string = fs$8.readFileSync(path$a.toNamespacedPath(jsonPath), 'utf8');
  } catch (error) {
...
function getPackageScopeConfig(resolved) {
  ...
  while (true) {
    ...
    const packageConfig = read$4(fileURLToPath$1(packageJSONUrl), {
      specifier: resolved
    });
...
function emitLegacyIndexDeprecation(url, packageJsonUrl, base, main) {
  ...
  } else if (path$a.resolve(packagePath, main) !== urlPath) {
    process$1.emitWarning(
...
function finalizeResolution(resolved, base, preserveSymlinks) {
...

  {
    ...
    resolved = pathToFileURL$1(real + (filePath.endsWith(path$a.sep) ? '/' : ''));
...
function packageResolve(specifier, base, conditions) {
  ...
  do {
    ...
    // Package match.
    const packageConfig = read$4(packageJsonPath, {base, specifier});
...
const CACHE_PATH = /* @__PURE__ */ join(os$1.tmpdir(), "unimport");
let CACHE_WRITEABLE;
...
function isWritable(filename) {
  try {
    accessSync(filename, constants$5.W_OK);
    return true;
  } catch {
    return false;
  }
...
async function enablePages(nuxt) {
  ...
  if (fs$8.existsSync(pathPageIndex)) {
    logger.warn("pages/index.vue already exists, skipping");
    return;
  }
  let appContent = fs$8.existsSync(pathApp) ? await fsp.readFile(pathApp, "utf-8") : void 0;
...
\``;
  ...
  const isMac = os$1.platform() === "darwin";
  logger.log([
